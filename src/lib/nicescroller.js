...
(function ($) {
    ...
    function _setDist(x, y) {
        if (this.dir == 0 && !this.cfg.inertia) {
            if (x > 0) {
                x = 0;
            } else if (x < (this.jScrollBox.width() - this.jBox.width()) * -1) {
                x = (this.jScrollBox.width() - this.jBox.width()) * -1;
            }
        } else if (this.dir == 1 && !this.cfg.inertia) {
            if (y > 0) {
                y = 0;
            } else if (y < (this.jScrollBox.height() - this.jBox.height()) * -1) {
                y = (this.jScrollBox.height() - this.jBox.height()) * -1;
            }
        }
     ...
     */
    var _getXY = function (el) {
        var e = el.originalEvent ? el.originalEvent : el,
            touches = e.touches,
            x = 0,
            y = 0;
        if (touches) {
     ...
     */
    function _start(e) {
        if (this.cfg.ontouchstart && !this.cfg.ontouchstart.apply(this)) {
            return
        }
        this.cfg.ontouchstart&& this.cfg.ontouchstart();
    ...
    function _move(e) {
        ...
        if (!_currentScroller || !_currentScroller.touched) {
            return
        }
        ...
        if (!_isChecked) {
            if (Math.abs(dx) < Math.abs(dy) && dir === 0) {
                _isLock = false
            }
            _isChecked = true
        }
    ...
    function _end() {
        ...
        if (endTime < 300) {
            mx = dir === 1 ? {
                duration: 0,
                destination: 0
            } : getLocation(point.x, point.x + deltaX, endTime, _currentScroller.maxScrollWidth, wrapperSize.width, deceleration);
            my = dir === 0 ? {
                duration: 0,
                destination: 0
            } : getLocation(point.y, point.y + deltaY, endTime, _currentScroller.maxScrollHeight, wrapperSize.height, deceleration);
    ...
    function _scrollTo(x, y, time) {
        ...
        var dir = that.dir;
        _cancelAnimate.apply(this);
        if (dir === 0 && !that.cfg.inertia && (x === 0 || x === that.maxScrollWidth)) {
            _setDist.call(that, x, y);
        } else if (dir === 1 && !that.cfg.inertia && (y === 0 || y === that.maxScrollHeight)) {
            _setDist.call(that, x, y);
    ...
    function _bind() {
        var that = this;
        this.jScrollBox.on(ev.start, function (e) {
            _start.call(that, e);
        });
        if (!_bound) {
          this.jScrollBox.on(ev.move, _move).on(ev.end, _end).on(ev.cancel, _end);
            _bound = true;
        }
    }
     ...
     */
    function _unbind() {
      this.jScrollBox.off(ev.move, _move).off(ev.end, _end).off(ev.cancel, _end);
        _bound = false;
    }
    ...
    function _init() {
        ...
        _isChecked = false;
        _scrollerCount = 0;
        this.jScrollBox = (this.cfg.scrollBox&&this.jBox.find(this.cfg.scrollBox))||this.jBox.children().eq(0);
