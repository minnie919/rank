...
(function (win, $) {
  ...
  var support = {
    // 触摸
    touch: (win.Modernizr && win.Modernizr.touch === true) || (function () {
  ...
  sS.prototype.init = function (options) {
    ...
    function fnGetSlideDistance() {
      ...
      $li.each(function (i) {
        if(me.opts.type == 'adslide'){
          fnTranslate(me, $(this), (me._slideDistance+10*window.innerWidth/750) * (i + num),true);
        } else {
          fnTranslate(me, $(this), me._slideDistance * (i + num),true);
        }
      });
    }
  };
  ...
  function fnAutoSlide(me) {
    ...
    if(me.canCallBack){
        me.opts.dataCallBack(me._index, me._liLength, me.$el);
    }
  ...
  // 指定轮播
  sS.prototype.goTo = function (i, noCallBack,isClick) {
    var me = this;
    fnSlide(me, i, '.3', noCallBack,isClick);
  };

  // 轮播方法
  function fnSlide(me, go, num, noCallBack,isClick) {
    // 判断方向
    if (typeof go === 'number') {
      ...
      if(me._index === go){
        ...
        if ((arguments[1] !== '') && (!noCallBack)) {
          me.canCallBack = true;
          me.opts.callback(me._index, me._liLength, me.$el,isClick);
        }
       return;
     }
      me._index = go;
